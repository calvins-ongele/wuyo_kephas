...
class CustomFunctions
{
    ...
    public static function maskEmail2($email, $how_many = 4)
    {
        if (strlen($email) < $how_many) {
            return ''; //"Invalid phone number";
        }

        return substr($email, 0, $how_many) . '***' . substr($email, -12);
    }
    ...
    public static function maskEmail($email, $how_many = 4)
    {
        ...
        return substr($parts[0], 0, $how_many) . "***" . '@' . $parts[1];
    }
    ...
    public static function pdfTemplate($data)
    {
