...
class MyApp_Model extends Model
{
    ...
    public function update_company_images() {
		...
		$b = $this->_update('company', "{$_POST['col']}", 'c_ID', [$img,  1]);
		echo $this->_ms(false, "<span class='text-success'>".json_decode($b)->msg."</span>");
    }
...
// 			CustomFunctions::SendMail($_POST['email'], "Welcome to  {$this->_company()['c_name']}" , $body, $this->_company() );
    ...
    public function manage_vehicle_types() {
       
        $file_name = "icon-".rand().'.jpg';
        $destPath = "assets/system/" . $file_name;
        
        if ($_POST['action'] == 'insert') {
            
            if ( empty($_POST['title']) || empty($_POST['cost']) || empty($_POST['min']) || empty($_FILES['icon']['name']) ) {
                die($this->_ms(1, "You must include title, icon, cost per km and minimum fare"));
            }
            
            move_uploaded_file($_FILES['icon']['tmp_name'], $destPath);
            
            echo $this->_insert('vehicle_types', 'vt_title, vt_cost_per_km, vt_icon, vt_min_fare', [ $_POST['title'], $_POST['cost'], $file_name,  $_POST['min'] ] );
                
        } else if ($_POST['action'] == 'update') {
            
            if ( empty($_POST['title']) || empty($_POST['cost']) || empty($_POST['min'])   ) {
                die($this->_ms(1, "You must include title, cost per km and minimum fare"));
            }
            
            if( !empty($_FILES['icon']['name']) ) {
            
                move_uploaded_file($_FILES['icon']['tmp_name'], $destPath);
                $this->_update('vehicle_types', ' vt_icon', 'vt_ID', [  $file_name,  $_POST['id']  ] );
            }
            
            echo $this->_update('vehicle_types', 'vt_title, vt_cost_per_km, vt_min_fare', 'vt_ID', [ $_POST['title'], $_POST['cost'],  $_POST['min'], $_POST['id'] ] );
        
            
        } else  if ( $_POST['action'] == 'delete' ) { 
            
            echo $this->_delete('vehicle_types', 'vt_ID', [ $_POST['id']]);
            if (file_exists( "assets/system/{$_POST['image']}" ) ) {  unlink( "assets/system/{$_POST['image']}" ); }
        }
	...
	public function subscribe() {
	    if (!empty($_POST['email'])) {
            ...
            $data = $this->_get('subscribers', 's_email', [ $_POST['email'] ] );
			...
			if (CustomFunctions::validEmail($_POST['email'])) { 
                ...
                $data = $this->_insert('subscribers', 's_email, s_date', [  $_POST['email'], date("Y-m-d")  ] );
