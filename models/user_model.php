...
class User_Model extends Model {
    ...
    public function users($id = '') {
        ...
        $search = $_GET['q'] ?? '';
        $search_type = 'x';
        if (CustomFunctions::validEmail($search)) $search_type = 'email';
       
        
        return json_decode($this->curl('rider/get', ['search_type'=> $search_type, 'val'=>$search ]), 1)['data'] ?? [];  
        return [$this->_get("users " )[0], $this->_get("users order by user_ID desc {$this->pagination()}" )[1] ];
    }
    public function getemails($email='') {
       
      return json_decode($this->curl('emails', ['email'=>$email, 'rand'=>rand() ]), 1);
    }
